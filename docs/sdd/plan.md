...
## 13. Installer Traceability Addendum

`install.sh` is intentionally external to the app architecture described above. Its bounded workflow is: validate macOS and required host commands/versions; reject an existing destination; clone `https://github.com/ozymand1as/firebird_test.git`; run `npm ci`, `bundle install`, and `bundle exec pod install --project-directory=ios`; then build the existing `FirebirdPosts` simulator scheme without launching it. Stage failures stop later work and retain a newly created partial destination. README guidance supplies the post-install two-terminal Metro/iOS commands; this script does not run them.
